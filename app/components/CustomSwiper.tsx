    ...
    <div className="container mx-auto md:max-w-7xl mt-7 relative">
      ...
      {useInternalNavigation && (
        <>
          <button
            ref={navPrevButton}
            className={`prev-button absolute top-1/2 transform -translate-y-1/2 left-3 z-10 hover:bg-gray-100 bg-white rounded-full w-9 h-9 flex items-center justify-center shadow-sm ${
              isBeginning ? "hidden" : ""
            }`}
          >
          ...
          <button
            ref={navNextButton}
            className={`next-button absolute top-1/2 transform -translate-y-1/2 right-3 z-10 hover:bg-gray-100 bg-white rounded-full w-9 h-9 flex items-center justify-center shadow-sm ${
              isEnd ? "hidden" : ""
            }`}
          >
            {/* SVG for next button */}
